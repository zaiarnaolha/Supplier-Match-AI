...
function Filters() {
  ...
  return <div className={styles.filterFields}>
    <label><span>Категорія</span><Select value={category} onValueChange={setCategory}><SelectTrigger className={styles.filterSelect}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="coffee">Кава</SelectItem><SelectItem value="grocery">Бакалія</SelectItem></SelectContent></Select></label>
    <label><span>Країна / регіон</span><Select value={region} onValueChange={setRegion}><SelectTrigger className={styles.filterSelect}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="ukraine">Україна</SelectItem><SelectItem value="europe">Європа</SelectItem></SelectContent></Select></label>
    <label><span>Ціна</span><Select value={price} onValueChange={setPrice}><SelectTrigger className={styles.filterSelect}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="any">Будь-яка</SelectItem><SelectItem value="300">До 300 грн/кг</SelectItem><SelectItem value="500">До 500 грн/кг</SelectItem></SelectContent></Select></label>
    <label><span>MOQ</span><Select value={moq} onValueChange={setMoq}><SelectTrigger className={styles.filterSelect}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="any">Будь-яке</SelectItem><SelectItem value="10">До 10 кг</SelectItem><SelectItem value="50">До 50 кг</SelectItem></SelectContent></Select></label>
    <Button className={styles.clearFilters} variant="ghost" type="button" onClick={clearFilters}><RotateCcw aria-hidden="true" />Очистити фільтри</Button>
  </div>;
}
  ...
  return <article className={cn(styles.supplierCard, isSelected && styles.supplierCardSelected)}>
    <div className={styles.supplierSummary}>
      ...
      <div className={styles.summaryDetails}>
        ...
        <div className={styles.matchMeta}><p className={styles.breakdown}>{supplier.breakdown}</p><small>Оновлено: {supplier.updatedAt}</small></div>
      </div>
    </div>
    ...
    <div className={styles.supplierActions}>
      ...
      <Button type="button" onClick={() => navigate(`/app/suppliers/${supplier.id}`)}>Переглянути постачальника<ArrowRight aria-hidden="true" /></Button>
    </div>
  </article>;
}
